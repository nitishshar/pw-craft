...
When('I fill the full name with {string}', async function (this: PwCraftSession, value: string) {
  await this.action.fill(this.find.byTestId('full-name-input'), value);
});

Then('the full name field should contain {string}', async function (this: PwCraftSession, value: string) {
  await this.assert.inputValue(this.find.byTestId('full-name-input'), value);
});

When('I submit a valid demo form', async function (this: PwCraftSession) {
...
Then('I should see the form success toast', async function (this: PwCraftSession) {
  await this.assert.elementVisible(this.find.byTestId('success-toast'));
});

When('I select category {string}', async function (this: PwCraftSession, label: string) {
  await this.action.click(this.find.byTestId('category-select'));
  await this.page.getByRole('option', { name: label }).click();
});

Then('the category field should show {string}', async function (this: PwCraftSession, label: string) {
  await this.assert.textEquals(this.find.byTestId('category-value'), label);
});

When('I set the newsletter checkbox to checked', async function (this: PwCraftSession) {
  await this.page.getByRole('checkbox', { name: 'Newsletter' }).check();
});

When('I set the newsletter checkbox to unchecked', async function (this: PwCraftSession) {
  await this.page.getByRole('checkbox', { name: 'Newsletter' }).uncheck();
});

Then('the newsletter checkbox should be checked', async function (this: PwCraftSession) {
  await this.assert.elementChecked(this.page.getByRole('checkbox', { name: 'Newsletter' }));
});

Then('the newsletter checkbox should be unchecked', async function (this: PwCraftSession) {
  await this.assert.elementUnchecked(this.page.getByRole('checkbox', { name: 'Newsletter' }));
});

When('I select shipping {string}', async function (this: PwCraftSession, value: string) {
  await this.action.click(this.find.byTestId(`shipping-${value}`));
});

Then('the shipping radio {string} should be checked', async function (this: PwCraftSession, value: string) {
...
When('I drag the first draggable item into the drop zone', async function (this: PwCraftSession) {
...
Then('the drop zone should contain dragged text', async function (this: PwCraftSession) {
  await this.assert.textEquals(this.find.byTestId('dropped-count'), '1');
});

When('I tab until the tooltip trigger is focused', async function (this: PwCraftSession) {
...
Then('the tooltip trigger should be focused', async function (this: PwCraftSession) {
  await this.assert.elementFocused(this.find.byTestId('tooltip-trigger'));
});

When('I upload a temporary text file', async function (this: PwCraftSession) {
...
Then('the file name display should not be empty', async function (this: PwCraftSession) {
  const text = await this.find.byTestId('file-name-display').innerText();
  expect(text.trim().length).toBeGreaterThan(0);
});

When('I attempt to submit the form without required fields', async function (this: PwCraftSession) {
  await this.action.click(this.find.byTestId('submit-form'));
});

Then('I should see a validation error for full name', async function (this: PwCraftSession) {
  await this.assert.elementVisible(this.find.byTestId('full-name-error'));
});

Then('I should see a validation error for email', async function (this: PwCraftSession) {
  await this.assert.elementVisible(this.find.byTestId('email-error'));
});

When('I slowly type {string} into the step size field', async function (this: PwCraftSession, value: string) {
...
Then('the step size field should contain {string}', async function (this: PwCraftSession, value: string) {
